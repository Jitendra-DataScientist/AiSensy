...
# Define the IndexPayload class that inherits from BaseModel
...
@app.get("/health")
async def root():
    return {"message": "AiSensy APIs healthy!"}


@app.post("/index")
async def ingest(index_payload: IndexPayload):
    payload = vars(index_payload)
    to_be_ingested = scrapper_obj.ingestion_check(payload['urls'])
    scraped_content = scrapper_obj.scrape_urls(to_be_ingested)
    if not scraped_content:
        return {"index": False, "helpText": f"urls already ingested: {payload['urls']}"}, 400
    else:
        indexer_obj.main_indexing(scrape_urls_res = scraped_content, scrapper_obj = scrapper_obj)
        return {"index": True}, 201
